...
def get_season_summary(seasonId: int, db: Session = Depends(get_db), farmer_id: int = Depends(verify_token)):
    ...
    upcoming_count = 0
    completed_count = 0
    overdue_count = season.overDueActivitiesCount if hasattr(season, "overDueActivitiesCount") else 0
    ...
    return {
        "seasonId": season.id,
        "totalEstimatedCostUgx": float(season.totalEstimatedCostUgx),
        "totalActualCostUgx": float(season.totalActualCostUgx),
